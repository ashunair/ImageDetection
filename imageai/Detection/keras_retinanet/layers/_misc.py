...
class Anchors(keras.layers.Layer):
    def __init__(self, size, stride, ratios=None, scales=None, *args, **kwargs):
        ...
        self.anchors = tf.Variable(utils_anchors.generate_anchors(
            base_size=size,
            ratios=ratios,
            scales=scales,
        ))
    ...
    def call(self, inputs, **kwargs):
        features = inputs
        features_shape = tf.keras.backend.shape(features)[:3]

        # generate proposals from bbox deltas and shifted anchors
        anchors = backend.shift(features_shape[1:3], self.stride, self.anchors)
        anchors = tf.tile(tf.expand_dims(anchors, axis=0), (tf.shape(features)[0], 1, 1))
...
class NonMaximumSuppression(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        indices = tf.range(tf.shape(classification)[0])
        selected_scores = []
        ...
        for c in range(int(classification.shape[1])):
            ...
            score_indices = tf.where(tf.greater(scores, self.score_threshold))
            score_indices = tf.cast(score_indices, 'int32')
            boxes_ = tf.gather_nd(boxes, score_indices)
            scores = tf.gather(scores, score_indices)[:, 0]
            ...
            selected_indices = tf.gather(score_indices, nms_indices)

            # mask original classification column, setting all suppressed values to 0
            scores = tf.gather(scores, nms_indices)
            scores = tf.tensor_scatter_nd_update(tf.zeros_like(classification[:, c]), selected_indices, scores)
            scores = tf.expand_dims(scores, axis=1)

            selected_scores.append(scores)

        # reconstruct the (suppressed) classification scores
        classification = tf.concat(selected_scores, axis=1)

        # reconstruct into the expected output
        detections = tf.concat([boxes, classification] + other, axis=1)

        return tf.expand_dims(detections, axis=0)
